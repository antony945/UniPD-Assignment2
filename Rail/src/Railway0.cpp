	...
	for (int i = 0; i < trains.size(); i++) {
		for (int j = i + 1; j < trains.size(); j++) {
			...
			if ((trains[i].getLeft() == trains[j].getLeft()) && (absoluteValue(trains[i].getCurrentDistance(), trains[j].getCurrentDistance()) < MIN_DIS)) {
				...
				else if (trains[i].getCurrentSpeed() < getCurrentSpeed[j].getCurrentSpeed() && trains[j].getCurrentDistance() < trains[i].getCurrentDistance()) {
